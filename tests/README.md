...
| `test_main_integration.py` | 端到端集成：用最小化 stub 模拟 AstrBot 框架接口，验证检测流程（含 QQ OneBot 合并转发递归展开、图片检测、AI 语义检测批量审核的按数量触发/超时兜底/手动触发/插件关闭前收尾）、撤回/警告/通知动作、分群配置覆盖、访问控制白名单黑名单、各管理指令的实际效果 |
